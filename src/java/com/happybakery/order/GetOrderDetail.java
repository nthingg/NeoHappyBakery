...
public class GetOrderDetail extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            Order order = null;
            User ordererOrderDetail = null;
            User storeOrderDetail = null;
            
            try {
                order = OrderDAO.getOrderById(orderId);
                listDetail = OrderDAO.getDetailsById(orderId);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            
            try {
                ordererOrderDetail = UserDAO.getUserById(order.getUserId());
                storeOrderDetail = UserDAO.getUserById(order.getStoreId());
            } catch (Exception ex) {
                ex.printStackTrace();
            }
     
            session.setAttribute("ordererOrderDetail", ordererOrderDetail);
            session.setAttribute("storeOrderDetail", storeOrderDetail);
