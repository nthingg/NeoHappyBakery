...
import com.happybakery.dto.DeliveryInfor;
...
public class UserDAO {
    ...
    public static DeliveryInfor getDeliveryInfo(int userId) throws Exception {
        DeliveryInfor deliver = null;
        Connection cn = DBUtils.makeConnection();
        if (cn != null) {
            String sql = "select UserID, ReceiverName, ReceiverPhone, Address\n"
                    + "from UserDelivery\n"
                    + "where UserID = ?";
            ...
            if (table != null && table.next()) {
                String ReceiverName = table.getString("ReceiverName");
                String ReceiverPhone = table.getString("ReceiverPhone");
                String Address = table.getString("Address");
                deliver = new DeliveryInfor(userId, ReceiverName, ReceiverPhone, Address);
            }
            cn.close();
        }
        return deliver;
    ...
    public static boolean insertDeliveryInfor(int userId, String fullname, String phone, String address) throws Exception {
        int result = 0;
        Connection cn = DBUtils.makeConnection();
        if (cn != null) {
            String sql = "DELETE FROM UserDelivery \n"
                    + "WHERE UserID = ?";
            PreparedStatement pst = cn.prepareStatement(sql);
            pst.setInt(1, userId);
            pst.executeUpdate();
            
            sql = "insert into UserDelivery (UserID, ReceiverName, ReceiverPhone, Address) \n"
                    + "values (?, ?, ?, ?)";
    ...
    public static void changeStatusAccount(String email, int status) {
        Connection cn = null;
        try {
            cn = DBUtils.makeConnection();
            if (cn != null) {
    ...
    public static ArrayList<User> getAllActiveUser() {
        ArrayList<User> list = new ArrayList<>();
        Connection cn = null;
        try {
            cn = DBUtils.makeConnection();
            if (cn != null) {
                String sql = "select UserID, UserName, UserMail, UserPassword, UserPhone, UserBDay, UserAddress, UserGender, UserType, UserStatus, UserImg\n"
                        + "from Users\n"
                        + "where UserType = 'USER' and UserStatus = 1";
                PreparedStatement pst = cn.prepareStatement(sql);
                ResultSet table = pst.executeQuery();
                if (table != null) {
                    while (table.next()) {
    ...
    public static ArrayList<User> getAllActiveStores() throws Exception {
        ArrayList<User> list = new ArrayList<>();
        Connection cn = DBUtils.makeConnection();
        if (cn != null) {
            String sql = "select UserID, UserName, UserMail, UserPassword, UserPhone, UserBDay, UserAddress, UserGender, UserType, UserStatus, UserImg\n"
                    + "from Users\n"
                    + "where UserType = 'STORE' and UserStatus =1";
            PreparedStatement pst = cn.prepareStatement(sql);
            ResultSet table = pst.executeQuery();
            if (table != null) {
                while (table.next()) {
