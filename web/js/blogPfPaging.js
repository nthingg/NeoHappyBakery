    ...
    const content = product.map((item, index) => {
        if (index >= startBlog && index < endBlog) {
            ...
            if (item.loginedid === item.userid) {
                html += '<a href="#" class="blog-delete" onclick="deleteBlog('+ item.id +')"><i class="fa fa-trash"></i></a>';
                html += '<a href="ManageBlog?actiontype=update&blogId='+ item.id +'" class="blog-update"><i class="fa fa-wrench"></i></a>';
            }
            html += '</div>';
            return html;
        }
