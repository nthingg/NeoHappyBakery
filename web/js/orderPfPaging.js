    ...
    const content = product.map((item, index) => {
        if (index >= startOrder && index < endOrder) {
            ...
            if (item.checkstore == 'STORE') {
                html += '<a class="order-more">Order by: <a class="order-author" href="GetUserProfile?checkedUserId=' + item.userid + '">' + item.username + '</a></a>';
            } 
            if (item.checkstore == 'USER') {
                html += '<a class="order-more">Store: <a class="order-author" href="GetUserProfile?checkedUserId=' + item.storeid + '">' + item.storename + '</a></a>';
            } 
            html += '</div>';
            html += '</div>';
        }
