...
function renderProduct(product) {
    ...
    const content = product.map((item, index) => {
        if (index >= start && index < end) {
            ...
            html += '<p class="ingredient-category">Category: ' + item.category + '</p>';
            html += '<p class="ingredient-category">Available: ' + item.quantity + '</p>';
            if (item.sale != '') {
                html += '<div class="ingredient-sale">-'+ item.sale +'%</div>';
                let totalPrice = item.price - (item.price * item.sale)/100;
                html += '<p class="ingredient-price lnth">' + item.price + ' VND</p>';
            ...
            if (item.loginedid !== item.pfid) {
                ...
                html += '<input class="quantity" min="0" max="1000" name="quantity" value="0" type="number">';
                html += '<button onclick="this.parentNode.querySelector(\'input[type=number]\').stepUp()" class="plus"></button>';
                html += '</div>';
                html += '<input type="hidden" name="storeId" value="'+ item.storeid +'">';
                html += '<input type="hidden" name="availId" value="'+ item.availid +'">';
                if (item.quantity == 0) {
                    html += '<button id="add-cart" value="addtocart" style="background-color: black;" name="action" class="add-to-cart">Add to cart</button>';
                } else {
                    html += '<button type="submit" id="add-cart" value="addtocart" name="action" class="add-to-cart">Add to cart</button>';
                }
                html += '</form>';
            }
            html += '</div>';
            if (item.loginedid === item.pfid) {
                html += '<a href="#" class="ingredient-delete" id="delete-ingredient" onclick="deleteIngredient('+ item.availid +')"><i class="fa fa-trash"></i></a>';
                html += '<a href="ManageIngredient?availId='+ item.availid +'" class="ingredient-update"><i class="fa fa-wrench"></i></a>';
            }
            html += '</div>';
            return html;
        }
