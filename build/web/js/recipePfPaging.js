    ...
    const content = product.map((item, index) => {
        if (index >= startRecipe && index < endRecipe) {
            ...
            if (item.loginedid === item.userid) {
                html += '<a href="#" class="recipe-delete" onclick="deleteRecipe('+ item.id +')"><i class="fa fa-trash"></i></a>';
                html += '<a href="ManageRecipe?CreateOrUpdate=2&recipeId='+ item.id +'" class="recipe-update"><i class="fa fa-wrench"></i></a>';
            }
            html += '</div>';
            return html;
        }
