...
let SpecArr = window.sessionStorage.getItem('SpecArray');
let checkOutArr = JSON.parse(SpecArr);
console.log(checkOutArr);
let productArr = [];
productArr = checkOutArr;
    ...
    const content = product.map((item, index) => {
        if (index >= start && index < end) {
            ...
            if (item.sale != '') {
                ...
                html += '<p class="ingredient-price saling"><span class="org-price">'+ item.price +'</span> ' + totalPrice + ' VND</p>';
            }
            if (item.sale == '') {
                html += '<p class="ingredient-price">' + item.price + ' VND</p>';
            }
                html += '<p class="storename">Store: <a href="GetUserProfile?checkedUserId='+ item.storeid +'" class="seller">'+ item.storename +'</a></p>';
            if (item.usertype == 'USER') {
                ...
                html += '<button type="submit" value="addtocart" name="action" class="add-to-cart">Add to cart</button>';
                html += '</form>';
            }
            html += '</div>';
            html += '</div>';
